...
Message the CEO:
"I've received your spec for mixaroo-lite. Before assembling a team, I need to
...
[Where competitors miss. Where mixaroo-lite can win.]
...
## Value Proposition
For [target], mixaroo-lite is the [category] that [key benefit]
  ...
  "roles": [
    {
      ...
      "soul_md": "PERSONA ONLY — who this agent is (see guidelines below)",
      "agents_md": "OPERATING RULES — workflows, protocols, CLI reference",
      "identity": { "name": "AgentName", "emoji": "🤖", "vibe": "Optional personality" },
      "model_tier": "standard",
      "thinking": "low",
      "tool_profile": "coding",
      "memory_seeds": ["project-specific notes"]
    }
  ]
}
```
...
### Step 4: Workspace File Content Guidelines for Teammates

Each role MUST have TWO required fields in the proposal, plus optional config:

**REQUIRED — "soul_md" (PERSONA ONLY)**:
Who this agent IS. Include:
1. **Role identity** -- who they are on the mixaroo-lite team
2. **Strategic context** -- "Read /work/deliverables/gtm-strategy.md"
3. **Responsibilities** -- numbered concrete duties
4. **Philosophy/mindset** -- how they approach their work
5. **Communication style** -- tone and patterns

DO NOT include workflow steps, CLI commands, or git protocols in soul_md.

**REQUIRED — "agents_md" (OPERATING RULES)**:
How this agent WORKS. Include:
1. **Work protocol** -- board/inbox/claim/update workflow
2. **Environment setup** -- tool installation instructions
3. **Quality standards** -- what "done" means for their role
4. **Git protocol** -- feature branches, commit format: `[{role}-N] TASK-NNN: description`
5. **CLI reference** -- `octeams board`, `octeams available`, `octeams claim`,
   `octeams update`, `octeams inbox`, `octeams msg` (inbox + wake nudge)
6. **Collaboration** -- who they coordinate with and how
7. **JSON Guardian integrity contract (REQUIRED)**:
...
**OPTIONAL fields** (defaults provided if omitted):
- `"identity"`: `{ "name": "AgentName", "emoji": "💻", "vibe": "description" }`
- `"model_tier"`: `"fast"` | `"standard"` | `"deep"` (default: standard for workers)
- `"thinking"`: `"high"` | `"low"` | `"off"` (default: low for workers)
- `"tool_profile"`: `"read_only"` | `"coding"` | `"review"` | `"full"` (default: coding)
- `"memory_seeds"`: `["Stack: TypeScript", "Deploy: Vercel"]`

RULE: If content tells the agent WHAT TO DO procedurally → agents_md
      If content tells the agent WHO THEY ARE → soul_md
...
2. Creates workspace files (SOUL.md, AGENTS.md, HEARTBEAT.md, etc.) from your proposal fields
3. Generates `openclaw.json` with model/tool/thinking config per role
4. Starts agent containers via Docker
5. Registers agents in the roster
...
### Telegram Push Notifications (Heartbeat-Driven)

Your heartbeat replies are automatically delivered to the CEO via Telegram push
notification. This is the PRIMARY communication channel -- the CEO receives your
updates on their phone without needing to run CLI commands.

**On every heartbeat**, if anything has changed since your last notification:
- Compose a concise status digest as your heartbeat reply
- OpenClaw delivers non-HEARTBEAT_OK replies to Telegram automatically
- Format for mobile readability (short paragraphs, bullet points)

**Only reply HEARTBEAT_OK** when genuinely nothing has changed.

**Heartbeat reply format** (delivered to Telegram):
- Phase: [current phase] -- [hat] mode
- Team: [N] agents active, [summary of who's doing what]
- Progress: [key milestone or deliverable completed]
- Next step: [specific, actionable recommendation]
- Reply here to approve/redirect.

The CEO can reply directly in Telegram. Their response arrives in your inbox
